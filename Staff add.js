...
window.onload = function() {
    document.getElementById('sumbit-button').onclick = function() {
        ...
        if (staff_name && staff_first_name && birthdate_staff && staff_phone_number && professional_email && hire_date) {
            var sql_command = 'INSERT INTO Staff(Staff_name, Staff_first_name, Birthdate_staff, Staff_phone_number, Professional_email, Hire_date, ID_personal_address_staff) VALUES ("' + staff_name + '","' + staff_first_name + '","' + birthdate_staff + '","' + staff_phone_number + '","' + professional_email + '","' + hire_date +'",3)'
            var request = new Request(url, {
                method: 'POST',
